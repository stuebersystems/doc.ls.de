...
2. Markieren Sie die zu exportierenden Schüler (Markierung analog Excel: 
   * STRG+A für alle, 
   * oder anklicken, STRG halten weiteren anklicken für mehrere einzelne Zeilen 
   * oder auch einen Block: Ersten Schüler anklicken, Shift halten, Letzten Schüler nklicken
3. Geben Sie im Export-Assistenten den Exportordner an und wählen `Schüler` oder `Bewerber` aus. Wählen Sie zusätzlich, ob bei Schlüsselfeldern das Kürzel oder der Schlüssel ausgegeben werden soll.
4. Starten Sie mit `Fertigstellen` den Export.
