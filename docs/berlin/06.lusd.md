...
Schüler werden in der LUSD angelegt und können in eine *.CSV exportiert werden. Diese Datei kann nach MAGELLAN importiert werden, damit die Schülerdatensätze dort weitergepflegt werden können.
...
Diese Werte werden unter `Schüler > Daten1` für die Zuordnung beim LUSD-Import im Feld `Verkehrssprache`verwendet. 
Zuätzlich wird das Verzeichnis - unabhängig vom LUSD-Import - auch hinter dem Feld `Muttersprache` verwendet.
...
Die von LUSD für den Import zur Verfügung gestellte Datei ist eine CSV-Datei mit einem festgelegten Format. Die Felder des Formats und deren Abbildung auf die Felder in MAGELLAN 11 sind in der anliegenden Datei „Datenset_LUSD_Magellan_HF_06052024.FinaleUmsetzung.xlsx“ beschrieben.

Spaltenkopf| Beispielinhalt|Anmerkung
--|--|--
Schueler_LokalerCode | ea7Aw5d3fY88 | `GUIDExtern`, wird bei der Überprüfung verwendet, existiert bereits ein Datensatz mit diesem Wert, wird der Import dieser Zeile übersprungen
...
Schueler_Notfallnummer | 01234567 | Die Notfallnummer wird dem 1. angelegten Schülerkontakt als `Tel. (Privat)` hinterlegt.
Schueler_Email | max@web.de | `Schüler > Daten1 > Email`
Schueler_Geschlecht | m | `Schüler > Daten1 > Geschlecht`<br/>Es werden männlich, weiblich oder divers als Werte übernommen. `Ohne Angabe` wird als leeres Feld übernommen.
Schueler_Geburtsdatum | 13.06.2006 | `Schüler > Daten1 > Geboren am`
...
Schueler_Familiensprache | Sonstige | `Schüler > Daten1 > Verkehrssprache`
...
Ansprechpartner_1_Ort | Berlin-Charlottenburg-Nord | Berlin-Charlottenburg-Nord | Bei Werten, die mit `Berlin` beginnen,  wird der Wert `Berlin` unter `Kontakte > Daten > Ort` egänzt, der Teil nach dem Bindestrich hinter Berlin wird unter `Kontakte > Daten > Ortsteil` ergänzt. Anderenfalls wird der Wert nur als `Ort` ergänzt.
...
Ansprechpartner_2_Ort | Berlin-Charlottenburg-Nord | Berlin-Charlottenburg-Nord | Berlin-Charlottenburg-Nord | Bei Werten, die mit `Berlin` beginnen,  wird der Wert `Berlin` unter `Kontakte > Daten > Ort` egänzt, der Teil nach dem Bindestrich hinter Berlin wird unter `Kontakte > Daten > Ortsteil` ergänzt. Anderenfalls wird der Wert nur als `Ort` ergänzt.
...
Wenn Daten eingelesen werden, wird versucht als erstes den Schüler anhand der Felder `Vorname`, `Nachname`, `Geburtsdatum` zu finden. Ist der Schüler einmal daran erkannt worden, wird dem Schüler eine `GUIDExtern` zugewiesen, anhand dessen er später immer wieder identifiziert werden kann.
...
Gibt es einen Schüler mit einer ID (MAGELLAN-Feld `ID`), welche mit dem Feld `MAGELLAN_ID` aus der CSV-Datei übereinstimmt, dann wird der Schüler in MAGELLAN aktualisiert und es wird ihm die LUSD-ID zugewiesen. Der Schüler gilt damit als importiert.
...
2. Wenn nein, dann Schüler als neuen Schüler (Vagabunden) in MAGELLAN anlegen. Der Schüler gilt damit als importiert.
...
Im zweiten Fall (erkannte Doubletten) wird eine neue CSV-Datei angelegt, die alle erkannte Doubletten enthält. Diese Datei bekommt den originalen Importdateinamen plus eine Ergänzung und wird parallel zum Importdatei gespeichert.
Diese Datei muss manuell nachbearbeitet werden.

1. Öffnen Sie die Datei, schauen die verbliebenen Datensätze an.
2. Suchen Sie bitte die korrekten Datensätze in MAGELLAN anhand weiterer Daten, wie der Adresse o.ä.!
3. Tragen Sie die ID dieser Datensätze bitte in der Doublettendatei in der Spalte `MAGELLAN_ID`. Gehen Sie so bitte für alle weiteren Datensätze vor und speichern Ihre Änderung.
4. Importieren Sie diese Datei bitte erneut, die Schüler werden jetzt mit der LUSD-ID gekennzeichnet und können bei erneuten Importen wieder erkannt werden.

### Manuelle Nacharbeit der Schulen

Wurden Doubletten erkannt, werden diese in einer separaten CSV-Datei gespeichert, die neben Ihrer Importdatei abgelegt wird. Sie prüft bitte in MAGELLAN, welcher Schüler es ist, trägt die MAGELLAN_ID in diese CSV-Datei nach und starten den Import dieser Datei erneut.
