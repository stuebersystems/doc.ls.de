...
Sie benötigen für die Schnittstelle mindestens die Version MAGELLAN 11.0.5 und auf den Rechnern, auf denen der Import genutzt werden soll, bitte je Platz die MAGELLAN.CLI installieren.
...
#### MAGELLAN.CLI Installation

Bitte installieren Sie auf den Geräten, auf denen der Import ausgeführt werden soll, die MAGELLAN.CLI.

Nutzen Sie ein 32-Bit oder 64-Bit-Betriebssystem?

Wählen Sie `Start > Einstellungen > System > Info` aus. Unter Gerätespezifikationen > Systemtyp sehen Sie, ob Sie eine 32-Bit-Version oder eine 64-Bit-Version von Windows verwenden. Das Downloadpakte für die für den Übertrag notwendige Anwendung MAGELLAN.CLI finden Sie nachstehend, bitte wählen Sie das Installationpaket für 64 oder 32 Bit aus:

+ [MAGELLAN.CLI für Windows 64 Bit](https://download.stueber.de/bin/de/magellan-cli/magellan.cli-x64-de.msi)
+ [MAGELLAN.CLI für Windows 32 Bit](https://download.stueber.de/bin/de/magellan-cli/magellan.cli-x86-de.msi)
...
Bitte lesen Sie beide Keys-Dateien über das Modul `MAGELLAN Administrator > Datenaustausch > Kataloge (Schlüsselverzeichnisse) importieren` ein, eine allgmaine Anleitung für den Import finden unter [Schlüsselverzeichnisse](https://doc.ls.stueber.de/schluesselverzeichnisse/#allgemeine-schlusselverzeichnisse-importieren)
