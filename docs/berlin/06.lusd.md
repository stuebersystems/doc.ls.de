...
Gibt es bereits einen Schüler mit der LUSD-ID in MAGELLAN? Falls ja, dann wird der Schüler in MAGELLAN aktualisiert. Der Schüler gilt damit als importiert.

### Zweite Prüfung

Gibt es einen Schüler mit einer ID, welche mit dem Feld "MAGELLAN_ID" aus der CSV-Datei übereinstimmt, dann wird der Schüler in MAGELLAN aktualisiert und es wird im die LUSD-ID zugewiesen. Der Schüler gilt damit als importiert.

### Dritte Prüfung

Alle noch ausstehenden Schüler werden wie folgt abgearbeitet:
Gibt es in der CSV-Datei mehrere Schüler mit identischer Kombi aus Vorname, Nachname, Geburtsdatum?
...
Im zweiten Fall (erkannte Doubletten) wird eine neue CSV-Datei angelegt, die alle erkannte Doubletten enthält. Diese Datei muss manuell nachbearbeitet werden.
